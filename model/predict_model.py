...
def load_mappings_names(file_path):
    ...
    with open(file_path, mode='r') as file:
        ...
        for row in csv_reader:
            mapping[row['name'].strip()] = str(row['identifier'])
    return mapping
...
def load_or_train_model( model_file,scaler_file, dataset_file, train_start_date="01-01-2000", train_end_date="30-06-2024"):
    ...
    if os.path.exists(model_file) and os.path.exists(scaler_file):
        ...
        to_remove = ["player_id", "match_id", "date", "dates", "role", "batting_points", "bowling_points", "fielding_points", 
                     "total_fantasy_points", "fielding_points_ewma", "Team_batting_points_avg_5", "Team_bowling_points_avg_5"] + \
                     [f"opponentplayer{i}" for i in range(1, 12)] + [f"player{i}" for i in range(1, 11)]
        input_cols = [x for x in df.columns if x not in to_remove]
        target_cols = ["total_fantasy_points"]
        
        train_idx = df[(df['dates'] <= train_end_dates) & (df['dates'] > train_start_dates)].index
...
def prepare_columns(df):
    to_remove = ["player_id", "match_id", "date", "dates", "role", "batting_points", "bowling_points", 
                 "fielding_points", "total_fantasy_points", "fielding_points_ewma", "Team_batting_points_avg_5", 
                 "Team_bowling_points_avg_5"] + [f"opponentplayer{i}" for i in range(1, 12)] + \
...
def generate_predictions(format, team1_players, team2_players, test_date="01-07-2024"):
    # Load the dataset
    dataset_file = f"../../data/interim/{format}_final_dataset/{format}_final_dataset_after_drop.csv"
    df = pd.read_csv(dataset_file)
    ...
    for team_name, team_players in teams.items():
        ...
        opponent_aggregate = opponent_rows[[
            'batting_points_ewma', 'bowling_points_ewma', 
            'batting_points_avg_5', 'bowling_points_avg_5'
        ]].sum().to_dict()
        ...
        for player_id in team_players:
            ...
            nearest_row = player_rows.iloc[(player_rows['date'] - test_date).abs().argsort()[:1]]
            
            if nearest_row.empty:
                player_features = {'player_id': player_id}
                player_features.update({feature: None for feature in [
                    'batting_points_avg_3', 'bowling_points_avg_3', 
                    'batting_points_avg_5', 'bowling_points_avg_5',
                    'batting_points_avg_10', 'bowling_points_avg_10', 
                    'batting_points_ewma', 'bowling_points_ewma', 
                    'OpponentTeam_batting_points_ewma', 'OpponentTeam_bowling_points_ewma',
                    'OpponentTeam_batting_points_avg_5', 'OpponentTeam_bowling_points_avg_5']})
            else:
                ...
                player_features.update({
                    'OpponentTeam_batting_points_ewma': opponent_aggregate['batting_points_ewma'],
                    'OpponentTeam_bowling_points_ewma': opponent_aggregate['bowling_points_ewma'],
                    'OpponentTeam_batting_points_avg_5': opponent_aggregate['batting_points_avg_5'],
                    'OpponentTeam_bowling_points_avg_5': opponent_aggregate['bowling_points_avg_5']
                })
...
def generate_predictions_with_model( team1_players, team2_players,format, test_date="01-07-2024", model_file_path="path/to/model.pkl"):
    # Step 1: Generate the features DataFrame
    predictions_df = generate_predictions(format, team1_players, team2_players, test_date)
    ...
    team1_points = {pid: player_points[pid] for pid in team1_players if pid in player_points}
    team2_points = {pid: player_points[pid] for pid in team2_players if pid in player_points}
    ...
    top_11_dict = {k: float(v) for k, v in top_11_dict.items()}  # Convert to native float

    return top_11_dict


def generate_shap_values_for_players(
    top_11_players,names_id_to_name, players_id_to_name, format, test_date="01-07-2024", model_file_path="path/to/model.pkl"
):
    # Step 1: Generate the features DataFrame
    predictions_df = generate_predictions(format, team1_players, team2_players, test_date)
    ...
    for i, player_id in enumerate(player_ids):
        ...
        shap_details = {feature: round(shap_values[i, j], 4) for j, feature in enumerate(feature_names)}
        ...
        top_features = sorted(
            [(k, v) for k, v in shap_details.items() if k not in ['player_id', 'predicted_points']],
    ...
    top_players = sorted(player_shap_results, key=lambda x: x['predicted_points'], reverse=True)[:11]
    
    # Step 12: Convert to the desired output format
    results = {}
    for player in top_players:
        player_features = {k: v for k, v in player.items() if k not in ['player_id', 'predicted_points']}
...
def generate_explainability(player_id, features, api_key):
    ...
    # Step 2: Fetch the player name and role using player_id
    ...
    # Step 3: Format the features into a user-friendly string
    features_list = "\n".join(
        [f"  {i+1}. {key}: {value:+.4f}" for i, (key, value) in enumerate(features)]
    )

    # Step 4: Define the context and message
    ...
    # Step 5: Generate the explanation using Groq
    ...
    # Step 6: Print the generated explanation
    # print(chat_completion.choices[0].message.content)
    return (player_id,chat_completion.choices[0].message.content)
...
def parallel_generate_explanations(top_features):
    ...
    with ThreadPoolExecutor(max_workers=len(API_KEYS)) as executor:
        ...
        for i, (player_id, features) in enumerate(top_features.items()):
            ...
            futures.append(executor.submit(generate_explainability, player_id, features, api_key))
...
# Example usage
team1_players = ["7ed9fd56","5bdcdb72","d18f9182",'8dd02a98','896d78ad','c5928dec','9868bc75','0f12f9df','469ea22b','a12e1d51','ea0cdc12',]
team2_players = ['8305580c','39f01cdb','a343262c','d2a6c0e6','cca50cd6','7ca5e05d','99b75528','e087956b','ffe699c0','49b6c09f','29b89ae8']
# model_file_path = f"../model/model_{format}_30-06-2024.pkl"
format="ODI"
model_file_path=f"../../model/model_{format}_30-06-2024.pkl"
top_11_players = generate_predictions_with_model(team1_players, team2_players,format='ODI',  test_date="01-07-2024", model_file_path=model_file_path)
# print(top_11_players)6
top_features = generate_shap_values_for_players(top_11_players, names_id_to_name, players_id_to_name, format='ODI', test_date="01-07-2024", model_file_path=model_file_path)

print(json.dumps(top_11_players))
API_KEYS = [
    "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"
]

explanations = parallel_generate_explanations(top_features)
# print(explanations)